...
class Interface(QMainWindow, task_1_interface.Ui_MainWindow, QOpenGLWidget):
    ...
    def button_add(self):
        global part_list
        global dt
        emit = Coordinate(float(self.c_x.toPlainText()), float(self.c_y.toPlainText()))
    ...
    def button_add_random(self):
        ...
        global dt
        part_list.append(
            Particle(Coordinate(random.randint(-2500, 2500), random.randint(-2500, 2500)),
                     Velocity(random.randint(-5, 5) / 10000.0,random.randint(-5, 5) / 10000.0), random.randint(300, 1500),
                     [random.randint(0, 255) / 255.0, random.randint(0, 255) / 255.0, random.randint(0, 255) / 255.0],
                     dt * random.randint(100, 10000)))
        self.gl_sys.update()
    ...
    def combobox_numberChoice(self):
        ...
        if self.count_combo.currentIndex() != 5:
            ...
            part_list = []
            if self.count_combo.currentIndex() != 0:
                part_list.append(Particle(Coordinate(0, 0), Velocity(0, 0), 10000, (1, 0, 1), dt * 1000))
            for i in range(1, part_count):
                part_list.append(Particle(
                    Coordinate(random.randint(-2500, 2500), random.randint(-2500, 2500)),
                    Velocity(random.randint(-5, 5) / 2000.0, random.randint(-5, 5) / 2000.0), random.randint(300, 1500),
                    [random.randint(0, 255) / 255.0, random.randint(0, 255) / 255.0, random.randint(0, 255) / 255.0],
                    dt * random.randint(100, 10000)))
        else:
            self.zoom = 300
            ...
            part_count = 10
            part_list = []
            part_list.append(Particle(Coordinate(0, 0), Velocity(0, 0), 332900, [1, 1, 0], dt ** 2))
            part_list.append(
                Particle(Coordinate(0.387, 0), Velocity(0, 47870), 0.055, [139 / 255.0, 69 / 255.0, 19 / 255.0],
                         dt ** 2))
            part_list.append(Particle(Coordinate(0.7233, 0), Velocity(0, 35020), 0.815,
                                      [255 / 255.0, 160 / 255.0, 122 / 255.0], dt ** 2))
            part_list.append(
                Particle(Coordinate(1, 0), Velocity(0, 29760), 1, [30 / 255.0, 144 / 255.0, 255 / 255.0],
                         dt ** 2))
            part_list.append(
                Particle(Coordinate(1.524, 0), Velocity(0, 24130), 0.107, [255 / 255.0, 127 / 255.0, 0], dt ** 2))
            part_list.append(
                Particle(Coordinate(5.2, 0), Velocity(0, 13070), 318, [233 / 255.0, 150 / 255.0, 122 / 255.0],
                         dt ** 2))
            part_list.append(
                Particle(Coordinate(10.0, 0), Velocity(0, 9670), 95.0, [255 / 255.0, 222 / 255.0, 173 / 255.0],
                         dt ** 2))
            part_list.append(
                Particle(Coordinate(19.23, 0), Velocity(0, 6840), 14.6, [0, 0, 127 / 255.0], dt ** 2))
            part_list.append(
                Particle(Coordinate(30.07, 0), Velocity(0, 5480), 17.1, [30 / 255.0, 100 / 255.0, 255 / 255.0],
                         dt ** 2))
            part_list.append(
                Particle(Coordinate(39.48, 0), Velocity(0, 4750), 0.002, [255 / 255.0, 218 / 255.0, 185 / 255.0],
                         dt ** 2))
        self.timer.start(dt)
        self.gl_sys.update()
    ...
    def calculate(self):
        ...
        if self.count_combo.currentIndex() != 5:
            ...
            for partic in part_list:
                ...
                if (partic.alive):
                    x_n.append(partic.x)
                    y_n.append(partic.y)
                    coordinate = [Coordinate(x, y) for x, y in zip(x_n, y_n)]
                    u_n.append(partic.u)
                    v_n.append(partic.v)
                    m_n.append(partic.m)
        ...
        if self.method_combo.currentIndex() == 0:
            ...
            ax_n = []
            ay_n = []
            for cx, cy in zip(x_n, y_n):
                part = Coordinate(cx, cy)
                ax = []
                ay = []
            ...
            ax_n1 = []
            ay_n1 = []
            for cx, cy in zip(x_n1, y_n1):
                part = Coordinate(cx, cy)
                ax = []
                ay = []
                for x, y, m in zip(x_n1, y_n1, m_n):
                    c = Coordinate(x, y)
                    module = part.module(c)
                    if module > 0:
            ...
            if self.count_combo.currentIndex() != 5:
                for i in range(length):
                    coordinate = Coordinate(x_n1[i], y_n1[i])
                    velocity = Velocity(u_n1[i], v_n1[i])
                    part_list.append(Particle(coordinate, velocity, m_n[i], col_n[i], t_n[i]))
            else:
                for i in range(length):
                    coordinate = Coordinate(x_n1[i] / AEM, y_n1[i] / AEM)
                    velocity = Velocity(u_n1[i], v_n1[i])
        ...
        if self.method_combo.currentIndex() == 1:
            ...
            v_n1 = [v + 0.5 * (an + an1) * timerStep
                    for v, an, an1 in zip(v_n, ay_n, ay_n1)]
            coordinate_verlet = Coordinate(0,0)
            velocity_verlet = Velocity(0,0)
            part_list_verlet = []
            if self.count_combo.currentIndex() != 5:
                for i in range(length):
                    coordinate_verlet = Coordinate(x_n1[i], y_n1[i])
                    velocity_verlet = Velocity(u_n1[i], v_n1[i])
                    part_list_verlet.append(Particle(coordinate_verlet, velocity_verlet, m_n[i], col_n[i], t_n[i]))
            else:
                for i in range(length):
                    coordinate_verlet = Coordinate(x_n1[i] / AEM, y_n1[i] / AEM)
                    velocity_verlet = Velocity(u_n1[i], v_n1[i])
            ...
            for i in range(length):
                ...
                u_n1.append(result2[1,i,2])
                v_n1.append(result2[1,i,3])
            part_list = []
            if self.count_combo.currentIndex() != 5: 
                for i in range(length):
                    coordinate = Coordinate(x_n1[i], y_n1[i])
            ...
            if (len(norma)<100):
                norma_cur=0
                for i in range(length):      
                    norma_cur=norma_cur+((part_list[i].velocity.u-part_list_verlet[i].velocity.u)**2+(part_list[i].velocity.v-part_list_verlet[i].velocity.v)**2
                            +(part_list[i].coordinate.x-part_list_verlet[i].coordinate.x)**2+(part_list[i].coordinate.y-part_list_verlet[i].coordinate.y)**2)**0.5
    ...
    def g(self, list_of_data, time_span, list_of_mass, N):
        G = 6.67 * 10 ** (-11)
        mass_of_funct = np.zeros(4 * N)
        for i in range(0, N):
            f1 = list_of_data[4 * i + 2: 4 * i + 4]
            f2 = np.zeros(2)
            for j in range(0, N):
                if (i != j):
                    f2 += G * list_of_mass[j] * (
                                list_of_data[4 * j:4 * j + 2] - list_of_data[4 * i:4 * i + 2]) / nlg.norm(
                        list_of_data[4 * j:4 * j + 2] - list_of_data[4 * i:4 * i + 2], 2) ** 3
            mass_of_funct[4 * i:4 * i + 2] = f1
            mass_of_funct[4 * i + 2:4 * i + 4] = f2
    ...
    def draw(self):
        ...
        if self.count_combo.currentIndex() != 5:
            for i in range(len(part_list)):
                if (abs(part_list[i].x) < 2500) & (abs(part_list[i].y) < 2500):
                    ...
                    glTranslatef(part_list[i].x, part_list[i].y, 50)
            ...
            for i in range(len(part_list)):
                if (abs(part_list[i].x) < 2500) & (abs(part_list[i].y) < 2500):
                    ...
                    glLightModelfv(GL_LIGHT_MODEL_AMBIENT, part_list[i].color)
                    glMaterialfv(GL_FRONT_AND_BACK, GL_SPECULAR, part_list[i].color)
                    glTranslatef(50 * part_list[i].x, 50 * part_list[i].y, 50)
                    gluQuadricDrawStyle(sphere, GLU_FILL)
                    if i == 0:
